...
public final class SequenceUtil {
	...
	/**
	 * 获取秒序列的16进制字符表示，但需系统自身保证分布式场景下的唯一性
	 * @return
	 */
	public static synchronized String getSerialBySecond(){
		try{
			Thread.sleep(1000);
		}catch(InterruptedException e){
			LOG.error(e.getMessage(), e);
		}
		return getCurrentTime(8);
	}
	...
	public static void main(String args[]){
		...
		for(int i = 0 ; i < 10; i++){
			String serial = SequenceUtil.getSerialBySecond();
			s.add(serial);
